# Unbiased Walk-Forward Backtest — Phase 0 reference baseline (2026-08-22)

**This is the new reference baseline.** It supersedes
`docs/backtest_unbiased_postfix_2026-08-19.md` for every purpose.

**Prior numbers are non-comparable — they are not a regression, they measured a
different model.** The harness called `create_features(log)` with no
`team_stats`, so 17 of the 86 declared `FEATURE_COLS` were never built, and
`MLPredictor.predict` silently substitutes `0` for anything absent
(`nba_evaluator.py:3266-3270`). Every report up to and including
`postfix_2026-08-19` therefore measured a **69-feature** model. Supplying
opponent context builds **81** (the remaining 5 — `INJURIES_TEAM`,
`INJURIES_OPP` and the three `VEGAS_*` — are declared but never built by
`create_features` under any argument, and are removed in Phase 2). Counting the
two interactions that were present but pinned at constant zero
(`POSITION_x_OPP_DEF`, `POSITION_x_OPP_PACE`), **14 entries of the served
vector go from a hard-coded 0 to real signal**. Movement against the earlier
report is the instrument being repaired, not the model getting worse.

The opponent context fed in is **point-in-time**, rebuilt at every replay step
from team-games played strictly before that step's date (see caveat 1). A
season aggregate here would have leaked April into November.
...
| PRA | 44 | 606 | 7.64 | 7.82 | -0.41 | 9.74 | 7.12 | **+0.70** |
...
| 60-70% | 3501 | 64.8% | 53.9% | +10.9 |
| 70-80% | 1999 | 74.4% | 64.1% | +10.3 |
...
| 60-70% | 447 | 64.9% | 53.5% | +11.4 |
| 70-80% | 372 | 74.8% | 59.4% | +15.4 |
...
| Deni Avdija | 72 | 12 | PRA | 15.59 | -14.53 | 18.28 | 8.88 | +6.72 | 0.42 | 0.83 |
...
| Rudy Gobert | 72 | 12 | PRA | 11.02 | -8.98 | 12.86 | 6.50 | +4.52 | 0.17 | 0.58 |
...
| Stephen Curry | 70 | 10 | PRA | 12.51 | -4.38 | 15.74 | 8.29 | +4.22 | 0.40 | 0.90 |
